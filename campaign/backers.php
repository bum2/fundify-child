...
<div id="backers">
	...
	<?php if ( empty( $backers ) ) : ?>
		<p><?php _e( 'No backers yet for this language. Be the first!', 'fundify-child' ); ?></p>
	<?php else : ?>
		...
		<?php foreach ( $backers as $payment_id ) : ?>
			...
			<li class="backer">
				<?php echo get_avatar( $anon || !isset($user_info['email']) ? '' : $user_info[ 'email' ], 40 ); // bumbum add || isset ?>

				<div class="backer-info">
					<strong>
						<?php if ( $anon ) : ?>
							<?php _ex( 'Anonymous', 'Backer chose to hide their name', 'fundify-child' ); ?>
						<?php else : ?>
